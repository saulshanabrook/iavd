...
function nonEmptyEntries() {
    ...
    for (var i = 0; i < entries.length; i++) {
        var entry = entries[i];
        if (entry.className != "entry") continue;
...
function search(S, asPrefix, headWordOnly) {
    ...
    for (var l in visibleLetter) {
        visibleLetter[l] = false;
...
function searchQ() {
    return searchOb.search($('q').value);
}

function searchQall() {
    return searchOb.search($('q').value, false, false);
}

function searchQhead() {
    return searchOb.search($('q').value, false, true);
}

function searchQprefix() {
    return searchOb.search($('q').value, true, true);
}
...
function showErrs(b) {
    document.getElementById('errLetHead').style.display = 'block';
    document.getElementById('errLetTable').style.display = 'block';
